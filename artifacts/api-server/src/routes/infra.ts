...
- عند كتابة خطة أو وثيقة أو تحليل، اكتبها دائماً داخل code block واحد بصيغة markdown حتى يتمكن المالك من حفظها كملف
- اكتب الخطة بأسلوب احترافي تقني يتضمن:
  * عنوان رئيسي مع أيقونة ووصف
  * أقسام مرقمة بوضوح (1. المعمارية العامة، 2. دورة الإنشاء، الخ)
  * مخططات ASCII لتوضيح البنية والتدفقات مثل:
    \`\`\`
    +---------------------------+
    |  المستخدم (Browser)       |
    |  React + TypeScript       |
    ...
    +---------------------------+
    \`\`\`
  * تفاصيل كل مرحلة: الوكيل المسؤول، النموذج، المدخل، المخرج
  * أمثلة عملية وخطوات مرقمة [1] [2] [3]
  * شجرة الملفات والمسارات بتنسيق واضح
- لا تكتب خطة مختصرة أبداً — الخطة يجب أن تكون شاملة ومفصلة وجاهزة للتنفيذ`,
