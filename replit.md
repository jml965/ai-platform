...
- **Infrastructure Agents Panel:** A dedicated admin panel at `/infra` (accessible from Dashboard sidebar under "Admin Panel" with gold crown icon) for platform owner to manage and develop the platform itself via AI agents. Features 8 infrastructure agents: **System Director** (`infra_sysadmin`, Crown icon, gold styling, displayed separately at top of sidebar) — the supreme commander using 3 AI models (Claude + Gemini + o3-mini) in parallel + Governor merge for maximum accuracy; System Monitor (Gemini), Bug Fixer (Claude), Feature Builder (Claude), UI Updater (Claude), Database Manager (Claude), Security Guard (Claude), Deployment Agent (Gemini). Each agent has streaming chat with context-aware system prompts including full system blueprint. Director uses dedicated endpoint `POST /api/infra/director-stream` with parallel model execution and governor merge (supports Anthropic/Google/OpenAI providers); others use `POST /api/infra/chat-stream`. Status events (`type:"status"`) show as centered yellow pills during processing. Agents separated via `agentLayer` column (`"infra"` vs `"service"`). OpenAI reasoning models (o1/o3) automatically have temperature omitted. Backend routes: `GET /api/infra/agents`, `POST /api/infra/chat-stream`, `POST /api/infra/director-stream`, `POST /api/infra/clear-session`. **Agent Runtime Controls:** Search limiter (max 3 `search_text` per session), repeated query blocker (same query blocked), forced execution (auto-stop after 4 steps without edit), session summary logging (totalSteps, searchCount, hasEdited, queries logged to `ai_audit_logs`). **Source of Truth System:** Before editing text visible on site, agent must inspect DOM first via `get_page_structure`/`browse_page` to identify element (class/id/data-key), then trace to source (i18n/component/API), then edit. Edit results include `domInspected` flag and audit log tracks compliance. **Arabic Text Normalization:** `search_text` tool auto-generates variants for Arabic diacritics (أ/ا/إ/آ, ة/ه, ى/ي, ؤ/و, ئ/ي) — searches all forms automatically. Results include `matchedVariant` showing which form was found. **Anti-hallucination:** 8-law system prompt prevents using words not in user's message, blocks illogical questions, forbids fake success reports. **Controlled AI Runtime:** `TOOL_RISK_CONFIG` classifies 40 tools by risk (low/medium/high/critical). High/critical tools require admin approval via `ai_approvals` table. DB filtering blocks DROP/ALTER/TRUNCATE. Kill switch persisted in DB. Full audit trail in `ai_audit_logs`. **UI Verification:** After successful edit/write of UI files (.tsx/.jsx/.css/.html), system auto-takes screenshot_page and sends it to the agent for visual confirmation. Search results enriched with file-type labels and import-usage hints. Agent must report "ظاهر في الواجهة: نعم/لا" in final response. **Safe Deploy Chain (Law 8):** After edit → agent auto-requests git_push (requires approval, NO --force, auto backup tag) → CI/CD deploys to mrcodeai.com → deploy_status check → verify_production (fetches page HTML, checks text exists). Tools: `git_commit` (medium, no approval), `git_push` (high, approval required, creates backup-{timestamp} tag, uses `git push github master:main` without --force), `verify_production` (low, fetches mrcodeai.com HTML and checks for expected text), `rollback_deploy` (high, approval required, reverts to backup tag), `trigger_deploy`, `deploy_status`. DB write validation with pre/post SELECT checks.
